...
lob_traps1 <- mutate(lob_traps, DATE = as.Date(DATE, format = '%m/%d/%y')) # make date formats the same 
...
```{r}
# find the counts of lobsters per each site
lob_counts <- lob_size3 %>%
  group_by(SITE) %>%
  dplyr::summarise(count = length(SIZE))

# find the counts of traps per each site
